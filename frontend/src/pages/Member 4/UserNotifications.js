...
const STATUS_META = {
    OPEN:        { label: 'Open',        emoji: '🔵', pill: 'bg-blue-100 text-blue-700 border-blue-200',       icon: 'bg-blue-100 text-blue-600' },
    IN_PROGRESS: { label: 'In Progress', emoji: '🟡', pill: 'bg-amber-100 text-amber-700 border-amber-200',     icon: 'bg-amber-100 text-amber-600' },
    RESOLVED:    { label: 'Resolved',    emoji: '✅', pill: 'bg-emerald-100 text-emerald-700 border-emerald-200', icon: 'bg-emerald-100 text-emerald-600' },
    CLOSED:      { label: 'Closed',      emoji: '⚫', pill: 'bg-gray-100 text-gray-600 border-gray-200',        icon: 'bg-gray-100 text-gray-500' },
    REJECTED:    { label: 'Rejected',    emoji: '❌', pill: 'bg-red-100 text-red-700 border-red-200',           icon: 'bg-red-100 text-red-600' },
};
            ...
            <div className="max-w-2xl mx-auto px-4 py-10">
                ...
                <div className="mb-8">
                    <div className="flex items-center gap-3 mb-1">
                        ...
                        {unreadCount > 0 && (
                            <span className="ml-auto inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow shadow-indigo-200">
                                {unreadCount} new
                            </span>
                        )}
                    </div>
                ...
                <div className="flex items-center justify-between mb-5 gap-3 flex-wrap">
                    ...
                    {unreadCount > 0 && (
                        <button
                            onClick={markAllRead}
                            className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:text-purple-600 transition-colors px-3 py-1.5 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/20 flex items-center gap-1.5"
                        >
                    ...
                    <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl shadow-sm p-14 text-center">
                        ...
                        <p className="text-sm text-gray-400 dark:text-gray-500">
                            {filter === 'UNREAD'
                                ...
                                : 'Submit a ticket and you\'ll see updates here.'}
                        </p>
                    </div>
                            ...
                            return (
                                <div
                                    ...
                                    className={`group relative flex items-start gap-4 p-4 sm:p-5 rounded-2xl border cursor-pointer transition-all duration-200
                                        ${!notif.read
                                            ? 'bg-white dark:bg-gray-800 border-indigo-100 dark:border-indigo-900/50 shadow-sm shadow-indigo-100/50 hover:shadow-md hover:shadow-indigo-100'
                                            : 'bg-gray-50/80 dark:bg-gray-800/50 border-gray-100 dark:border-gray-700/50 hover:bg-white dark:hover:bg-gray-800'
                                        }`}
                                >
                                    {/* Unread strip */}
                                    {!notif.read && (
                                        <div className="absolute left-0 top-4 bottom-4 w-1 rounded-r-full bg-gradient-to-b from-indigo-600 to-purple-600" />
                                    )}
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-start justify-between gap-2 mb-1">
                                            ...
                                            <div className="flex items-center gap-2 flex-shrink-0">
                                                ...
                                                {!notif.read && <span className="w-2 h-2 rounded-full bg-indigo-600 flex-shrink-0 animate-pulse" />}
                                            </div>
                                        </div>
                                    ...
                                    {/* Chevron */}
                                    {notif.targetPath && (
                                        <svg className="flex-shrink-0 w-4 h-4 text-gray-300 group-hover:text-indigo-500 transition-colors mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
